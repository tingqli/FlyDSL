...
def composition(A, B):
    ...
    if isinstance(B, Layout):
        ...
        def _composition(sub_B):
            if sub_B.shape.is_leaf:
                ...
                for i in range(sub_B.rank):
                    S2, D2 = _composition(sub_B.sublayout(i))
                    if len(S2) == 1:
                        sub_RS.extend(S2)
                        sub_RD.extend(D2)
                    else:
        ...
        RD=[]
        for i in range(rank):
            rsub = composition(A.sublayout(i), B[i])
...
def rearrange_layouts(by_mode_layouts, passthrough_layouts, method=None):
    ...
    if method == "zipped":
        ...
        for l in by_mode_layouts:
            tile_layouts.append(l.sublayout(0))
            rest_layouts.append(l.sublayout(1))
...
def logical_divide(A, B, tiler_mode=None):
    ...
    if isinstance(B, Layout):
        ...
        for i in range(A.rank):
            if i < len(B):
                # (Tile, Rest)
                div_layouts.append(logical_divide(A.sublayout(i), B[i]))
            else:
                # no tiler provided for this mode, keep it as is (the L,... part)
                keep_layouts.append(A.sublayout(i))
...
def logical_product(A, B, tiler_mode=None):
    ...
    if isinstance(B, Layout):
        ...
        for i in range(A.rank):
            if i < len(B):
                # (Tile, Rest)
                prod_layouts.append(logical_product(A.sublayout(i), B[i]))
            else:
                # no tiler provided for this mode, keep it as is (the L,... part)
                keep_layouts.append(A.sublayout(i))
...
def blocked_product(A, B):
    ...
    mode0 = result.sublayout(0)
    mode1 = result.sublayout(1)
    RS = []
    RD = []
    for i in range(A.rank):
